...
/**
 * Replaces the placeholder in a bundled stylesheet with the generated CSS,
 * at the offset the marker occupies.
 *
 ...
 * or `null` if the marker isn't present
 */
export const splice_css_at_placeholder = (source: string, generated_css: string): string | null => {
	...
	const block_start = source.lastIndexOf('}', decl.index) + 1; // 0 when the marker's rule is first
	const open_brace = source.indexOf('{', block_start);
...
export const vite_plugin_fuz_css = (options: VitePluginFuzCssOptions = {}): Plugin => {
	...
	return {
		...
		async load(id) {
			...
			// page) and splice the full generated CSS in at the marker's position,
			// which is where import order put this module.
		...
		generateBundle(_options, bundle) {
			...
			for (const chunk of Object.values(bundle)) {
				if (
					...
					const spliced = splice_css_at_placeholder(chunk.source, generated_css);
					if (spliced !== null) {
