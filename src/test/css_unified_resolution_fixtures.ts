...
/**
 * Helper to assert that patterns appear in order within a string.
 * Useful for testing CSS cascade order.
 *
 * @example
 * assert_order(result.base_css, 'color: blue', 'color: darkblue', 'color: navy');
 */
export const assert_order = (content: string, ...patterns: Array<string>): void => {
	const positions = patterns.map((p) => {
		const pos = content.indexOf(p);
		if (pos === -1) {
			throw new Error(`Pattern not found: "${p}"`);
		}
		return pos;
	});
	for (let i = 1; i < positions.length; i++) {
		if (positions[i]! <= positions[i - 1]!) {
			throw new Error(
				`Pattern "${patterns[i]}" (pos ${positions[i]}) should appear after "${patterns[i - 1]}" (pos ${positions[i - 1]})`,
			);
		}
	}
};
