...
add `.compact` for buttons, inputs, chips, and menu items

- add `.compact` composite class for tighter sizing by overriding sizing variables
- fix `.chip`, `.pane`, and `.panel` to use `var(--border_radius, var(--border_radius_xs))` fallback pattern so container overrides cascade
